...
public class ReadAdapter extends RecyclerView.Adapter<ReadAdapter.ViewHolder> {
    ...
    public void onBindViewHolder(@NonNull ViewHolder holder, int position) {
        ...
        holder.itemView.setOnLongClickListener(v -> {
            ...
            //action pada button update
            btnUpdate.setOnClickListener(v1 -> {
                updateData(new resultMahasiswa(edtID.getText().toString(), edtNIM.getText().toString(),
                        edtNama.getText().toString(), edtFakultas.getText().toString(),//mengambil nilai dr edtNama kemudia diubah ke string
                        edtJurusan.getText().toString()));
                dialog.dismiss();
            });
            //actin pada button delete
    ...
    private void deleteData(String id) {
        ...
        dialog = new ProgressDialog(view.getContext()); //dialog yang akan ditampilkan
        ...
        apiService = ApiClient.getClient().create(ApiService.class); //mengambil api pdkelas apiservice
        Call<resultResponse> resultResponseCall = apiService.delete(id); //memanggil respon dari result
        resultResponseCall.enqueue(new Callback<resultResponse>() {
            ...
            public void onResponse(Call<resultResponse> call, Response<resultResponse> response) {
                ...
                } else { //kondisi ketika tidak berhasil
                    dialog.dismiss();
                    Toast.makeText(view.getContext(), message, Toast.LENGTH_SHORT).show();
                }
            }
